...
fastify.get("/fotos", (request, reply) => {
  ...
  .then(responesFromJsonPlaceholderSite => {return responesFromJsonPlaceholderSite.json()})
  .then(responseAsJson =>{
    reply
      .code(200)
...
fastify.get("/photos/:id", (request, reply) => {
  ...
  fastify.get("/showphoto/:id", (request, reply) => {
    // #4 TODO:
    // Adapt the following code to attempt to retrieve
    // a single photo from JSONPlaceholder site
    // using fetch, and handle returned Promise using:
    ...
    .then(responseAsJson =>{
      reply
        ...
        .send(`<html><body><h1>${responseAsJson.title}</h1><img src= '${responseAsJson.url}'/></body></html>`);
      })
      .catch(err => {
        reply
        .code(404)
